	...
	dragScaleHandle(e){
...

		{
			...
			if (I) {
				...
				let toObjectSpace = new THREE.Matrix4().copy( this.selection[0].matrixWorld).invert();
		...
		if(this.selection.length === 1){
...

			{
				...
				for(let handleName of Object.keys(this.handles)){
					...
					let rotInv = new THREE.Matrix4().copy(rot).invert();
				...
				if(!this.dragging){
					...
					let tObject = new THREE.Matrix4().copy(tWorld).invert();
